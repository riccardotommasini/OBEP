package sr.obep.impl.parser.delp.parser;
...
import sr.obep.core.data.streams.EventStreamImpl;
import sr.obep.core.impl.parser.delp.declarations.*;
import sr.obep.impl.parser.sparql.SPARQLQuery;
...
import sr.obep.impl.pipeline.EPLFactory;
import sr.obep.impl.parser.delp.declarations.*;
import sr.obep.impl.parser.delp.program.CompositeEventImpl;
import sr.obep.impl.parser.delp.program.LogicalEventImpl;
...
public class OBEPParserOutput extends SPARQLQuery implements ProgramDeclaration {
    ...
    public ProgramImpl asProgram() {
        try {
            ...
            Set<EventStream> inputstreams = q.getStreamUris().stream().map(EventStreamImpl::new).collect(Collectors.toSet());
            ...
            Set<CompositeEvent> ces = compositeEvents.stream()
                    .map(ced -> {
                        ...
                        PatternStream patternStream = PatternStream.create(ced.toEpl());
                        return new CompositeEventImpl(ced.getName(), EPLFactory.toEPL(patternStream), normalforms, ced.getVariableAliases());
